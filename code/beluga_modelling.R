# ---------------------------------------------------------------------------------------------
#' Analysis file for the Beluga whales
#' 
#' 
...
# Preamble ------------------------------------------------------------------------------------
  ...
  rawRes <- readxl::read_xlsx("data/MM_Density_RES_Data.xlsx", sheet = "Beluga (P2)", na = "-")
  
  
  resGrid <- read_csv("data/Beluga_whale.csv") %>%
...
#' 
  ...
  table(belugaData$`Uncertainty measure`, useNA = "always")
  
  # check that where there is not a single measure, we do have a CI
  test <- belugaData %>% filter(is.na(`Uncertainty measure`)) 
  ...
  belugaData <- belugaData %>%
      mutate(workingLowerCI = qlnorm(0.025, logMu(Density, workingSE), logSigma(Density, workingSE)),
             ...
             blockID = paste(belugaData$`Survey ID`, belugaData$`Area/Segment`, sep = ":")
      )

  # sample for each of the area/segments collectively (as not independent)  
  
  dataList <- split(belugaData, belugaData$blockID)
  ...
  belugaSamples <- belugaData %>% left_join(sampleDF)
  
  
  belugaSamples <- belugaSamples %>% 
    select(-Density) %>%
    pivot_longer(names_to = "sampleID", values_to = "Density", V1:V600) 
  ...
  fittedMatrix <- matrix(fittedDF$Pred, ncol = 600)
  ...
  bootPlot <- ggplot(plottingDF) + 
    ...
    ggtitle("Density as function of RES", "Beluga whale: bootstrapped monotone spline fits") 
  
  bootPlot
  
  ggsave("docs/images/beluga_bootplot_600.png", units = "cm", width = 30, height = 20)
...
# Create RES grid predictions -----------------------------------------------------------------
  ...
  write.csv(predictionOutput, file = "data/beluga_predictions.csv", row.names = F)
