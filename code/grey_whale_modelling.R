# ---------------------------------------------------------------------------------------------
#' Analysis file for the Grey Whale
#' 
#' 
...
# Preamble ------------------------------------------------------------------------------------
  ...
  rawRes <- readxl::read_xlsx("data/MM_Density_RES_Data.xlsx", sheet = "Gray whale (P2)", na = "-")
  
  
  resGrid <- read_csv("data/Gray_whale.csv") %>%
    rename(RES = `Overall Probability`)
  
  # Coding of missing values are "-", several numeric fields interpreted as char
  
  greywhaleData <- rawRes %>%
...
#' 
  ...
  table(greywhaleData$`Uncertainty measure`, useNA = "always")
  
  # check that where there is not a single measure, we do have a CI
  test <- greywhaleData %>% filter(is.na(`Uncertainty measure`)) 
  ...
  greywhaleData <- greywhaleData %>%
      mutate(workingLowerCI = qlnorm(0.025, logMu(Density, workingSE), logSigma(Density, workingSE)),
             ...
             blockID = paste(greywhaleData$`Survey ID`, greywhaleData$`Area/Segment`, sep = ":")
      )

  # sample for each of the area/segments collectively (as not independent)  
  
  dataList <- split(greywhaleData, greywhaleData$blockID)
  ...
  greywhaleSamples <- greywhaleData %>% left_join(sampleDF)
  
  
  greywhaleSamples <- greywhaleSamples %>% 
    select(-Density) %>%
...
# Create RES grid predictions -----------------------------------------------------------------
  ...
  write.csv(predictionOutput, file = "data/predictions/greywhale_predictions.csv", row.names = F)
