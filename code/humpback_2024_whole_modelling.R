# ---------------------------------------------------------------------------------------------
#' Analysis file for the humpback data
...
# Preamble ------------------------------------------------------------------------------------
  ...
  source("code/tools.R")  
  speciesName <- "Humpback"  
  locationName <- "whole"

  # note missing values in spreadsheet indicated by "-" in some cases (cols J and K)

  rawRes <- read_csv("NE Atlantic Data/Humpback_whale.csv")
  
  
  resGrid <- read_csv("NE Atlantic Data/RES spreadsheets/Humpback whale (Megaptera novaeangliae) - Native range.csv")
...
#' 
  ...
  ggsave(paste0("docs/images/", speciesName, "_", locationName, "_bootplot_100.png"), units = "cm", width = 30, height = 20)
