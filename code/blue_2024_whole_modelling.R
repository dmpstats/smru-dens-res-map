# ---------------------------------------------------------------------------------------------
#' Analysis file for the blue whales data
...
# Preamble ------------------------------------------------------------------------------------
  ...
  rawRes <- read_csv("NE Atlantic Data/Blue_whale.csv")
  
  
  resGrid <- read_csv("NE Atlantic Data/RES spreadsheets/Blue whale (Balaenoptera musculus) - Native range.csv")
...
#' 
  ...
  bootPlot <- ggplot(plottingDF) + 
    ...
    ggtitle("Density as function of RES", "Blue whale: bootstrapped monotone spline fits") 
  
  bootPlot
  
  ggsave("docs/images/blue_whole_bootplot_100.png", units = "cm", width = 30, height = 20)
...
# Create RES grid predictions -----------------------------------------------------------------
  ...
  write.csv(predictionOutput, file = "data/predictions/blue_whole_predictions.csv", row.names = F)
