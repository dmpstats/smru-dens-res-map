...
# Preamble ------------------------------------------------------------------------------------
  ...
  rawRes <- readxl::read_xlsx("data/MM_Density_RES_Data.xlsx", sheet = "NPRW (P2)", na = "-")
  
  
  resGrid <- read_csv("data/North_Pacific_right_whale.csv", skip = 13) %>%
...
# Naive modelling -----------------------------------------------------------------------------
  ...
  NPRWGAM <- gam(Density ~ s(RES, bs = "cs"), data = NPRWData, family = gaussian(link = "log"))
  plot(NPRWGAM)
  plot(NPRWData$RES, predict(NPRWGAM, type = "response"))
  ...
  monGAM <- scam::scam(Density ~ s(RES, bs = "mpi", fx = F, k = 50)-1, data = NPRWData)
  
  plot(monGAM)
  
  
  monGAM <- scam::scam(Density ~ s(RES, bs = "mpi", fx = F, by = year)-1, data = NPRWData)
...
#' 
  ...
  table(NPRWData$`Uncertainty measure`, useNA = "always")
  
  # check that where there is not a single measure, we do have a CI
  test <- NPRWData %>% filter(is.na(`Uncertainty measure`)) 
  ...
  NPRWData <- NPRWData %>%
      mutate(workingLowerCI = qlnorm(0.025, logMu(Density, workingSE), logSigma(Density, workingSE)),
             ...
             blockID = paste(NPRWData$`Survey ID`, NPRWData$`Area/Segment`, sep = ":")
      )

  # sample for each of the area/segments collectively (as not independent)  
  
  dataList <- split(NPRWData, NPRWData$blockID)
  ...
  NPRWSamples <- NPRWData %>% left_join(sampleDF)
  
  
  NPRWSamples <- NPRWSamples %>% 
    select(-Density) %>%
    pivot_longer(names_to = "sampleID", values_to = "Density", V1:V500) 
  
  
  NPRWList <- split(NPRWSamples, NPRWSamples$sampleID)
  ...
  fittedList <- lapply(NPRWList, gamFit, inRES = data.frame(RES = seq(0, 1, by = 0.01))) 
...
# Create RES grid predictions -----------------------------------------------------------------
  ...
  write.csv(predictionOutput, file = "data/predictions/NPRW_predictions.csv", row.names = F)
