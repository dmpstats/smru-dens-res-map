# ---------------------------------------------------------------------------------------------
#' Analysis file for the Ringed Seal
#' 
#' 
...
# Preamble ------------------------------------------------------------------------------------
  ...
  rawRes <- readxl::read_xlsx("data/MM_Density_RES_Data.xlsx", sheet = "Ringed seal (P1)", na = "-")
  
  
  resGrid <- read_csv("data/Ringed_seal.csv") %>%
    rename(RES = `Overall Probability`)
  
  # Coding of missing values are "-", several numeric fields interpreted as char
  
  ringedsealData <- rawRes %>%
...
#' 
  ...
  table(ringedsealData$`Uncertainty measure`, useNA = "always")
  
  # check that where there is not a single measure, we do have a CI
  test <- ringedsealData %>% filter(is.na(`Uncertainty measure`)) 
  any(is.na(test$UpperCI))
  
  ringedsealData <- ringedsealData %>% 
  ...
  ringedsealData <- ringedsealData %>%
    mutate(`Uncertainty value` = ifelse(str_detect(`Uncertainty measure`, "% CV density"), `Uncertainty value`/100, `Uncertainty value`), 
           `Uncertainty measure` = ifelse(str_detect(`Uncertainty measure`, "% CV for abundance"), "CV", `Uncertainty measure`),
            `Uncertainty value` = ifelse(`Uncertainty value` == 3000, 3/186*Density, `Uncertainty value`), # hacks as need abundance to convert
            `Uncertainty value` = ifelse(`Uncertainty value` == 4000, 4/119*Density, `Uncertainty value`), # hacks as need abundance to convert
            `Uncertainty measure` = ifelse(str_detect(`Uncertainty measure`, "SE for abundance"), "SE", `Uncertainty measure`),
           `Uncertainty measure` = ifelse(str_detect(`Uncertainty measure`, "SE for density"), "SE", `Uncertainty measure`),
  ...
  ringedsealData <- ringedsealData %>%
      mutate(workingLowerCI = qlnorm(0.025, logMu(Density, workingSE), logSigma(Density, workingSE)),
             ...
             blockID = paste(ringedsealData$`Survey ID`, ringedsealData$`Area/Segment`, sep = ":")
      )

  # sample for each of the area/segments collectively (as not independent)  
  
  dataList <- split(ringedsealData, ringedsealData$blockID)
  ...
  ringedsealSamples <- ringedsealData %>% left_join(sampleDF)
  
  
  ringedsealSamples <- ringedsealSamples %>% 
    select(-Density) %>%
    pivot_longer(names_to = "sampleID", values_to = "Density", V1:V500) 
  
  
  ringedsealList <- split(ringedsealSamples, ringedsealSamples$sampleID)
  ...
  fittedList <- lapply(ringedsealList, gamFit, inRES = data.frame(RES = seq(0, 1, by = 0.01))) 
...
# Create RES grid predictions -----------------------------------------------------------------
  ...
  write.csv(predictionOutput, file = "data/predictions/ringedseal_predictions.csv", row.names = F)
