# ---------------------------------------------------------------------------------------------
#' Analysis file for the Spotted Seal
#' 
#' 
...
# Preamble ------------------------------------------------------------------------------------
  ...
  rawRes <- readxl::read_xlsx("data/MM_Density_RES_Data.xlsx", sheet = "Spotted seal (P2)", na = "-")
  
  
  resGrid <- read_csv("data/Spotted_seal.csv") %>%
    rename(RES = `Overall Probability`)
  
  # Coding of missing values are "-", several numeric fields interpreted as char
  
  spottedsealData <- rawRes %>%
...
#' 
  ...
  table(spottedsealData$`Uncertainty measure`, useNA = "always")
  
  # check that where there is not a single measure, we do have a CI
  test <- spottedsealData %>% filter(is.na(`Uncertainty measure`)) 
  any(is.na(test$UpperCI))
  
  spottedsealData <- spottedsealData %>% 
  ...
  spottedsealData <- spottedsealData %>%
    mutate(`Uncertainty value` = ifelse(`Uncertainty value` == 24000, 24/240*Density, `Uncertainty value`), # hacks as need abundance to convert
      `Uncertainty value` = ifelse(`Uncertainty value` == 6000, 6/163*Density, `Uncertainty value`), # hacks as need abundance to convert
  ...
  spottedsealData <- spottedsealData %>%
      mutate(workingLowerCI = qlnorm(0.025, logMu(Density, workingSE), logSigma(Density, workingSE)),
             ...
             blockID = paste(spottedsealData$`Survey ID`, spottedsealData$`Area/Segment`, sep = ":")
      )

  # sample for each of the area/segments collectively (as not independent)  
  
  dataList <- split(spottedsealData, spottedsealData$blockID)
  ...
  spottedsealSamples <- spottedsealData %>% left_join(sampleDF)
  
  
  spottedsealSamples <- spottedsealSamples %>% 
    select(-Density) %>%
    pivot_longer(names_to = "sampleID", values_to = "Density", V1:V500) 
  
  
  spottedsealList <- split(spottedsealSamples, spottedsealSamples$sampleID)
 
  fittedList <- lapply(spottedsealList, gamFit, inRES = data.frame(RES = seq(0, 1, by = 0.01))) 
  ...
  ggplot(plottingDF) + 
    ...
    ggtitle("Fitted function", "Spotted seal: observed densities & monotone fit") 
...
# Create RES grid predictions -----------------------------------------------------------------
  ...
  write.csv(predictionOutput, file = "data/predictions/spottedseal_predictions.csv", row.names = F)
