...
# Preamble ------------------------------------------------------------------------------------
  ...
  rawRes <- readxl::read_xlsx("data/MM_Density_RES_Data.xlsx", sheet = "Walrus(P2)", na = "-")
  
  
  resGrid <- read_csv("data/Walrus.csv", skip = 13) %>%
    rename(RES = `Overall Probability`)
  
  # Coding of missing values are "-", several numeric fields interpreted as char
  
  walrusData <- rawRes %>%
    mutate(`Uncertainty value`= ifelse(`Uncertainty measure` == "SE for abundance/\r\nCV for abundance",
                                       str_extract(`Uncertainty value`, "(?<=\\n).+"), `Uncertainty value`)) %>%
...
#' 
  ...
  table(walrusData$`Uncertainty measure`, useNA = "always")
  
  # check that where there is not a single measure, we do have a CI
  test <- walrusData %>% filter(is.na(`Uncertainty measure`)) 
  ...
  walrusData <- walrusData %>%
    mutate(`Uncertainty measure` = ifelse(str_detect(`Uncertainty measure`, "CV"), "CV", `Uncertainty measure`), 
           ...
           `Uncertainty value` = ifelse(`Uncertainty value` == 166, 166/424*Density, `Uncertainty value`), # hacks as need abundance to convert
           `Uncertainty value` = ifelse(`Uncertainty value` == 846, 846/2047*Density, `Uncertainty value`), # hacks as need abundance to convert
           `Uncertainty value` = ifelse(`Uncertainty value` == 349, 349/509*Density, `Uncertainty value`), # hacks as need abundance to convert
           `Uncertainty value` = ifelse(`Uncertainty value` == 509, 509/1278*Density, `Uncertainty value`), # hacks as need abundance to convert
           `Uncertainty value` = ifelse(`Uncertainty value` == 317, 317/505*Density, `Uncertainty value`), # hacks as need abundance to convert
           `Uncertainty value` = ifelse(`Uncertainty value` == 9, 9/12*Density, `Uncertainty value`), # hacks as need abundance to convert
  ...
  walrusData <- walrusData %>%
      mutate(workingLowerCI = qlnorm(0.025, logMu(Density, workingSE), logSigma(Density, workingSE)),
             ...
             blockID = paste(walrusData$`Survey ID`, walrusData$`Area/Segment`, sep = ":")
      )

  # sample for each of the area/segments collectively (as not independent)  
  
  dataList <- split(walrusData, walrusData$blockID)
  ...
  walrusSamples <- walrusData %>% left_join(sampleDF)
  
  
  walrusSamples <- walrusSamples %>% 
    select(-Density) %>%
    pivot_longer(names_to = "sampleID", values_to = "Density", V1:V500) 
  
  
  walrusList <- split(walrusSamples, walrusSamples$sampleID)
  
 
  fittedList <- lapply(walrusList, gamFit, inRES = data.frame(RES = seq(0, 1, by = 0.01))) 
  ...
  ggplot(plottingDF) + 
    ...
    ggtitle("Fitted function", "Walrus: observed densities & monotone fit") 
...
# Create RES grid predictions -----------------------------------------------------------------
  ...
  write.csv(predictionOutput, file = "data/predictions/walrus_predictions.csv", row.names = F)
