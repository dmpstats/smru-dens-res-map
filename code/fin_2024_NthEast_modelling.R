...
# Preamble ------------------------------------------------------------------------------------
  ...
  finData <- rawRes %>% 
    rename(UpperCI = CI_95_high,
           LowerCI = CI_95_low) %>% 
    filter(Location == "Northeast Atlantic")
...
#' 
  ...
  ggsave("docs/images/fin_NthEast_bootplot_100.png", units = "cm", width = 30, height = 20)
...
# Create RES grid predictions -----------------------------------------------------------------
  ...
  write.csv(predictionOutput, file = "data/fin_NthEast_predictions.csv", row.names = F)
