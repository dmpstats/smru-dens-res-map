...
# Preamble ------------------------------------------------------------------------------------
  ...
  finData <- rawRes %>% 
    rename(UpperCI = CI_95_high,
           LowerCI = CI_95_low) %>% 
    filter(Location != "Southern hemisphere")
...
#' 
  ...
  ggsave("docs/images/fin_noSth_bootplot_100.png", units = "cm", width = 30, height = 20)
...
# Create RES grid predictions -----------------------------------------------------------------
  ...
  write.csv(predictionOutput, file = "data/fin_noSth_predictions.csv", row.names = F)
