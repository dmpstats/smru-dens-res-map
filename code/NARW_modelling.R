...
# Preamble ------------------------------------------------------------------------------------
  ...
  rawRes <- readxl::read_xlsx("data/MM_Density_RES_Data.xlsx", sheet = "NARW (P2)", na = "-")
  
  
  resGrid <- read_csv("data/North_Atlantic_right_whale.csv") %>%
...
#' 
  ...
  table(NARWData$`Uncertainty measure`, useNA = "always")
  
  # check that where there is not a single measure, we do have a CI
  test <- NARWData %>% filter(is.na(`Uncertainty value`)) 
  ...
  table(NARWData$`Uncertainty measure`)
  ...
  NARWData <- NARWData %>%
    mutate(`Uncertainty value` = ifelse(str_detect(`Uncertainty measure`, "CV"), `Uncertainty value`*Density, `Uncertainty value`),
           `Uncertainty measure` = ifelse(str_detect(`Uncertainty measure`, "CV"), "CV", `Uncertainty measure`), 
  ...
  NARWData <- NARWData %>%
      mutate(workingLowerCI = qlnorm(0.025, logMu(Density, workingSE), logSigma(Density, workingSE)),
             ...
             blockID = paste(NARWData$`Survey ID`, NARWData$`Area/Segment`, sep = ":")
      )

  # sample for each of the area/segments collectively (as not independent)  
  
  dataList <- split(NARWData, NARWData$blockID)
  
  set.seed(4835)
  
  sampleList <- lapply(dataList, function(q){sampleLN(q$Density[1], q$workingSE[1], 300)})
  ...
  NARWSamples <- NARWData %>% left_join(sampleDF)
  
  
  NARWSamples <- NARWSamples %>% 
    select(-Density) %>%
    pivot_longer(names_to = "sampleID", values_to = "Density", V1:V300) 
  ...
  fittedMatrix <- matrix(fittedDF$Pred, ncol = 300)
...
# Create RES grid predictions -----------------------------------------------------------------
  ...
  write.csv(predictionOutput, file = "data/predictions/NARW_predictions.csv", row.names = F)
