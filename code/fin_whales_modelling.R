# ---------------------------------------------------------------------------------------------
#' Analysis file for the brydes whales data
...
# Preamble ------------------------------------------------------------------------------------
  ...
  rawRes <- read_csv("NE Atlantic Data/Brydes_whale.csv")
  
  
  resGrid <- read_csv("NE Atlantic Data/RES spreadsheets/Bryde's whale (Balaenoptera brydei) - Native range.csv")
...
#' 
  ...
  fittedList <- future_map(sampleList, \(x) gamFit(x, inRES = data.frame(RES = seq(0, 1, by = 0.01))))
  ...
  bootPlot <- ggplot(plottingDF) + 
    ...
    ggtitle("Density as function of RES", "Brydes whale: bootstrapped monotone spline fits") 
  
  bootPlot
  
  ggsave("docs/images/brydes_whole_bootplot_100.png", units = "cm", width = 30, height = 20)
...
# Create RES grid predictions -----------------------------------------------------------------
  ...
  write.csv(predictionOutput, file = "data/predictions/brydes_whole_predictions.csv", row.names = F)
